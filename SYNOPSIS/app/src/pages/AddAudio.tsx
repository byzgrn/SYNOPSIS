...
const AddAudio = ({navigation}:navigationProp) => {
  ...
  const folder = `userAudioRecordings/${userId}`;
    ...
    return (
      <View style={{ padding: 50, backgroundColor:colors.darkbrown, flex:1, justifyContent:'center' }}>
        <View style={{ padding: 25, margin:20, marginBottom:50, backgroundColor:colors.brown, borderWidth:2, borderColor:colors.grayish, borderRadius:5 }}>
        ...
        {selectedFile && (
          <View style={{ marginTop: 20 }}>
            ...
            <Button text="Upload File" onPress={() => uploadFile({folder:folder,fileName:fileName, userId:userId})} />
          </View>
        )}
        
      </View>
      <View style={{ padding: 25, margin:20, backgroundColor:colors.brown, borderWidth:2, borderColor:colors.grayish, borderRadius:5 }}>
