...
const useProcessAudio = (fileName: string, folderName: string | any) => {
    ...
    useEffect(() => {
        if (!base64Audio) return; // Only run if base64Audio is set
        ...
        const processAudioContent = async () => {
            try {
                ...
                const result = await model.generateContent([
                    ...
                    {
                        text:  "Reply to my questions with exact and strict responses, with no additional information." + "Informal language, filler words, or a conversational tone in audio is not important." + "Is content educational in a way formal or informal does not matter?" + 
                          + "if not reply with 'No.' and highlight its reason."
        ...
        processAudioContent(); // Call the processing function
    }, [base64Audio]); // Run effect when base64Audio changes
    ...
    const process = async () => {
        try {
            ...
            setBase64Audio(base64String); // This will trigger the useEffect
        } catch (error) {
            console.log('Error processing sound:', error);
        }
    };
    ...
    const blobToBase64 = (blob: Blob): Promise<string> => {
        return new Promise((resolve, reject) => {
            ...
            reader.onloadend = () => {
                ...
                resolve(result.split(',')[1]); // Return base64 string without metadata
            };
